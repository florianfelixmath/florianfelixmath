...
${(a_i)}_{i \in \mathbb{N}}$_,
${(b_j)}_{j \in \mathbb{N}}$ in $M$ such that $M \vDash \phi(a_i,b_j)$ if and only if $i \leq j$. A theory with the order property encodes the order $(\mathbb N, \leq)$, which is a red flag -- as said before, this determines cuts, and that is too high a complexity. We will then say that $T$ is *stable* if it doesn't have the order property.
